...
def load_data():
    ...
    df = pd.read_csv("../../data/metr-la/clean_metrla.csv")
...
if __name__ == "__main__":
    ...
    Path("metr-la-data").mkdir(exist_ok=True)
    Path("metr-la-results").mkdir(exist_ok=True)
    ...
    X_raw, Y_raw = X.copy(), Y.copy() #pad_with_nan_to_make_grid(X.copy(), Y.copy())
    ...
    # slice_epoch = utils.datetime_str_to_epoch('2017-01-01 00:00:00')
    ...
    for i, fold in enumerate(FOLDS):
        ...
        with open(f'metr-la-data/{train_name}', 'wb') as file:
            pickle.dump(training_data, file)

        with open(f'metr-la-data/{pred_name}', "wb") as file:
            pickle.dump(prediction_data, file)


        with open(f'metr-la-data/{raw_name}', "wb") as file:
